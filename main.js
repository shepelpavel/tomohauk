...
`;
                if (
                    !fs.existsSync("/etc/nginx/sites-available/" + options.name)
                ) {
                    fs.writeFile(
                        configDir + "tmp_domain_nginx",
                        _config_nginx,
                        "utf8",
                        function (err, data) {
                            if (err) {
                                event.sender.send(
                                    "system-res",
                                    "error save tmp_domain_nginx file"
                                );
                                event.sender.send("system-res", err);
                                event.sender.send("loader-hide");
                            } else {
    ...
    if (fs.existsSync(_site_conf)) {
        ...
        fs.readFile(_site_conf, "utf8", function (err, data) {
            if (err) {
                event.sender.send(
                    "system-res",
                    "error open config file " + _site_conf
                );
                event.sender.send("system-res", err);
            } else if (data) {
                var _to_editor = {
...
ipcMain.on("delete_site_config", (event, site) => {
    fs.readFile(configFile, "utf8", function (err, data) {
        ...
        if (config) {
            ...
            if (
                fs.existsSync("/etc/apache2/sites-available/" + site + ".conf")
            ) {
                _exec +=
    ...
    if (fs.existsSync(_log_file)) {
        fs.readFile(_log_file, "utf8", function (err, data) {
            ...
            if (err) {
                event.sender.send(
                    "system-res",
                    "error open file /var/log/nginx/error.log"
                );
                event.sender.send("system-res", err);
            } else if (data) {
                var _to_editor = {
    ...
    if (fs.existsSync(_log_file)) {
        fs.readFile(_log_file, "utf8", function (err, data) {
            ...
            if (err) {
                event.sender.send(
                    "system-res",
                    "error open file /var/log/nginx/access.log"
                );
                event.sender.send("system-res", err);
            } else if (data) {
                var _to_editor = {
        ...
        if (config && options.repo.length > 0) {
            ...
            dir = exec(_exec, function (err, stdout, stderr) {
                if (err) {
                    ...
                    event.sender.send("clear-inputs-git", [
                        "git_clone",
                        "repo_name",
                    ]);
                    if (stdout) {
            ...
            fs.readdir(config.projects_path, (err, directories) => {
                ...
                directories.forEach(function (directoire, idx) {
                    ...
                    if (
                        directoire.charAt(0) !== "." &&
                        fs.existsSync(
                            config.projects_path + directoire + "/.git/"
                        )
                    ) {
                        var _exec =
                            "cd " +

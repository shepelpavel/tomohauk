...
// get theme from file
ipcMain.on("get_theme", (event, options) => {
  _result = JSON.parse(fs.readFileSync(configFile));
  event.sender.send("curent-theme", _result.theme);
});

// write theme to file
ipcMain.on("set_theme", (event, options) => {
  _settings = JSON.parse(fs.readFileSync(configFile));
  _settings.theme = options;
  var _json = JSON.stringify(_settings);
  fs.writeFile(configFile, _json, "utf8", function (err, data) {
    if (err) {
...
ipcMain.on("get_sites_enable", (event, options) => {
  ...
  if (fs.existsSync(_site_conf)) {
    fs.readdirSync(_site_conf).forEach(function (file) {
      _result.push(file.replace(".conf", ""));
    });
...
`;
      var _config_apache = `<VirtualHost ${options.name}:80>
...
`;
      if (
        !fs.existsSync("/etc/apache2/sites-available/" + options.name + ".conf")
      ) {
        fs.writeFile(
          configDir + "tmp_domain_apache",
          _config_apache,
          "utf8",
          function (err, data) {
            if (err) {
              event.sender.send(
                "system-res",
                "error save tmp_domain_apache file"
              );
              event.sender.send("system-res", err);
              event.sender.send("loader-hide");
            } else {
              var _exec = `sudo mv ${configDir}tmp_domain_apache /etc/apache2/sites-available/${options.name}.conf && sudo chown -R root:root /etc/apache2/sites-available/${options.name}.conf && sudo chmod 644 /etc/apache2/sites-available/${options.name}.conf && sudo a2ensite ${options.name}.conf && echo "${_hosts}" | sudo tee -a ${config.hosts_path} && sudo service apache2 restart`;
              dir = exec(_exec, function (err, stdout, stderr) {
                if (err) {
...
// open site config in editor
ipcMain.on("edit_site_config", (event, options) => {
  var _site_conf = "/etc/apache2/sites-available/" + options.file + ".conf";
  if (fs.existsSync(_site_conf)) {
  ...
  if (fs.existsSync(_log_file)) {
    fs.readFile(_log_file, "utf8", function (err, data) {
      ...
      if (err) {
        event.sender.send(
          "system-res",
          "error open file /var/log/apache2/error.log"
        );
        event.sender.send("system-res", err);
      } else if (data) {
        var _to_editor = {
  ...
  if (fs.existsSync(_log_file)) {
    fs.readFile(_log_file, "utf8", function (err, data) {
      ...
      if (err) {
        event.sender.send(
          "system-res",
          "error open file /var/log/apache2/access.log"
        );
        event.sender.send("system-res", err);
      } else if (data) {
        var _to_editor = {
    ...
    if (config) {
      fs.readFile(config.hosts_path, "utf8", function (err, data) {
        if (err) {
          event.sender.send(
            "system-res",
            "error open file " + config.hosts_path
          );
          event.sender.send("system-res", err);
        } else if (data) {
          var _to_editor = {
...
ipcMain.on("git_clone", (event, options) => {
  ...
  fs.readFile(configFile, "utf8", function (err, data) {
    ...
    if (config && options.repo.length > 0) {
      var _exec =
        "git clone " + options.repo + " " + config.projects_path + _name;
      dir = exec(_exec, function (err, stdout, stderr) {
        if (err) {
    ...
    if (config) {
      fs.readdir(config.projects_path, (err, directories) => {
        ...
        directories.forEach(function (directoire, idx) {
          ...
          if (
            directoire.charAt(0) !== "." &&
            fs.existsSync(config.projects_path + directoire + "/.git/")
          ) {
            var _exec =
              "cd " + config.projects_path + directoire + "/ && git cherry -v";
            dir = exec(_exec, function (err, stdout, stderr) {
              if (err) {
        ...
        event.sender.send(
          "system-res",
          "checked " + _checked.length + " folders"
        );
        event.sender.send(
          "git-status-push",
          "checked " + _checked.length + " folders\n"

  ...
  methods: {
    ...
    usePhpVer: function (event) {
      if (!event.target.classList.contains("enabled")) {
        this.php_use = $(event.target).text().replace(/\s/g, "");
      }
    },
      ...
      if (this.git_clone.replace(/\s/g, "").length > 0) {
        ...
        var _data = {
          ...
          repo_name: this.repo_name
            ? this.repo_name
            : this.git_clone.replace(/\s/g, "").split("/")[1].split(".")[0],
        };
...
ipcRenderer.on("curent-theme", (event, resp) => {
  if (resp) {
    var theme = document.createElement("link");
    theme.type = "text/css";
    theme.rel = "stylesheet";
    theme.href = "./../_assets/css/" + resp.replace(/\s/g, "") + ".css";
...
ipcRenderer.on("reload", (event, resp) => {
